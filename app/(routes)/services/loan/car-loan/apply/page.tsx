...
const carLoanSchema = z.object({
  ...
  // Car Details
  carType: z.enum(["Sedan", "Hatchback", "SUV", "MUV", "Other"]).optional(),
  year: z.preprocess(
    val => val === "" || val === undefined ? undefined : Number(val),
    z.number().min(1980, "Year must be after 1980").max(new Date().getFullYear(), "Year cannot be in future").optional()
  ).optional(),
  loanType: z.enum(["New Car", "Used Car", "Finance Only"]).optional(),
...
const resolver = zodResolver(carLoanSchema) as any;
...
export default function CarLoanApplication() {
  ...
  const onSubmit = async (data: CarLoanForm) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/car-loan`, {
        method: "POST",
  ...
  const numericRegister = (name: keyof CarLoanForm) => ({
    ...register(name, {
  ...
  <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100 py-30 px-4">
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-emerald-800">Car Loan Application</h1>
      </div>
        ...
        <section className="border-b border-emerald-100 pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select
                {...register("carType")}
                className={`w-full border ${errors.carType ? "border-red-500" : "border-gray-300"} rounded-md px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:outline-none transition`}
              >
                <option value="">SELECT</option>
            ...
            <div>
              ...
              <select
                {...register("year")}
                className={`w-full border ${errors.year ? "border-red-500" : "border-gray-300"} rounded-md px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:outline-none transition`}
              >
            ...
            <div>
              ...
              <select
                {...register("loanType")}
                className={`w-full border ${errors.loanType ? "border-red-500" : "border-gray-300"} rounded-md px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:outline-none transition`}
              >
                <option value="">SELECT</option>
