...
export default function PersonalAccidentInsurancePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    occupation: "",
    // Policy Requirements
    coverageType: [] as string[], // ["Individual", "Family Coverage"]
    sumInsured: "",
    policyTermYears: "",
    coverageOptions: [] as string[], // Accidental Death, PD, TD, Hospitalization
    // Existing Policy
  ...
  const handleChange = (e: any) => {
    ...
    if (type === "checkbox") {
      ...
      if (["coverageType", "coverageOptions", "insurerPrefs"].includes(name)) {
        setFormData((prev) => {
  ...
  const validateStep2 = () => {
    const e: Record<string, string> = {};
    if (formData.coverageType.length === 0) e.coverageType = "Select Individual/Family";
    if (!formData.sumInsured || Number(formData.sumInsured) <= 0) e.sumInsured = "Enter valid sum insured";
    if (!formData.policyTermYears || Number(formData.policyTermYears) <= 0) e.policyTermYears = "Enter term in years";
    setErrors(e); return Object.keys(e).length === 0;
  };
  const validateStep3 = () => {
    const e: Record<string, string> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/personal-accident-insurance`, { method: "POST", body: fd});
      ...
      setTimeout(() => router.push("/services/insurance"), 2000);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 py-30 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <motion.div className="flex items-center justify-between mb-8 p-6 bg-white rounded-2xl shadow-lg border border-green-200" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-green-900">Personal Accident Insurance</h1>
          <div className="w-16" />
        </motion.div>
        ...
        <motion.div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-green-200" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, type: "spring", stiffness: 100 }}>
          <div className="mb-8 text-center">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Personal Accident Insurance Form</h2>
            <p className="text-gray-600 mt-2">Fill in details as per your requirements.</p>
          </div>

          {/* Progress */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {["Personal Details","Policy Requirements","Existing Policy","Insurer Preference"].map((label, idx) => (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium text-gray-700 mb-1 flex items-center"><FaBriefcase className="mr-2 text-green-600"/>Occupation</label>
                    <input name="occupation" value={formData.occupation} onChange={handleChange} placeholder="Your occupation" className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-green-500" />
                  </div>
                </div>

                <div className="pt-4">
                  <button type="button" onClick={handleNext} className="w-full py-4 px-6 rounded-xl font-bold text-white bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-700 hover:to-emerald-800">Next: Policy Requirements</button>
                </div>
              </>
            )}
            ...
            {step === 2 && (
              <>
                <h3 className="text-lg font-bold text-green-900 mb-2">Policy Requirements</h3>
                <div className="space-y-6">
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-2 flex items-center"><FaShieldAlt className="mr-2 text-green-600"/>Coverage Type<span className="text-red-500 ml-1">*</span></label>
                    {errors.coverageType && <p className="text-sm text-red-600 mb-2">{errors.coverageType}</p>}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {["Individual", "Family Coverage"].map((opt) => (
                        <label key={opt} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                          <input type="checkbox" name="coverageType" value={opt} checked={formData.coverageType.includes(opt)} onChange={handleChange} className="h-4 w-4 text-green-600 border-gray-300 rounded" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-1 flex items-center">Sum Insured Required (₹)<span className="text-red-500 ml-1">*</span></label>
                      <input name="sumInsured" type="number" value={formData.sumInsured} onChange={handleChange} placeholder="e.g., 1000000" className={`w-full px-4 py-3 rounded-xl border ${errors.sumInsured?"border-red-500":"border-gray-300"} focus:ring-2 focus:ring-green-500`} />
                      {errors.sumInsured && <p className="text-sm text-red-600 mt-1">{errors.sumInsured}</p>}
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-1 flex items-center">Policy Term (years)<span className="text-red-500 ml-1">*</span></label>
                      <input name="policyTermYears" type="number" value={formData.policyTermYears} onChange={handleChange} placeholder="e.g., 20" className={`w-full px-4 py-3 rounded-xl border ${errors.policyTermYears?"border-red-500":"border-gray-300"} focus:ring-2 focus:ring-green-500`} />
                      {errors.policyTermYears && <p className="text-sm text-red-600 mt-1">{errors.policyTermYears}</p>}
                    </div>
                  </div>

                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-2 flex items-center"><FaClipboardList className="mr-2 text-green-600"/>Coverage Options</label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {["Accidental Death","Permanent Disability","Temporary Disability","Hospitalization Expenses"].map((opt) => (
                        <label key={opt} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
              ...
              <>
                ...
                <div className="space-y-6">
                  <label className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg">
                    ...
                    <span className="text-gray-700">I already have an existing personal accident insurance policy</span>
                  </label>
            ...
            {step === 4 && (
              <>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {["ICICI Lombard","HDFC Ergo","Bajaj Allianz","Reliance General","Star Health"].map((i) => (
