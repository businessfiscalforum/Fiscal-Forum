...
export default function WhatsAppCommunityWidget() {
  ...
  const [isLineDismissed, setIsLineDismissed] = useState(false);

  useEffect(() => {
    setIsMounted(true);
    const dismissed = sessionStorage.getItem("whatsapp_line_dismissed");
    if (dismissed === "true") {
      setIsLineDismissed(true);
    }
  }, []);
  ...
  const handleClose = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    setIsLineDismissed(true);
    sessionStorage.setItem("whatsapp_line_dismissed", "true");
  };

  if (!isMounted) return null;

  return (
    <div className={`whatsapp-community-widget-wrap ${isLineDismissed ? "icon-only" : ""}`}>
      <a
