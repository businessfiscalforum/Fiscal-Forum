    ...
    "Stock Broking": [
      {
        ...
        incentive: "Up to ₹300",
        brokerageSharing:
          "15% for 2 years. If working regularly and earning high brokerage, extended to lifetime.",
    ...
    "Saving A/C": [
      {
        ...
        incentive: "₹200",
        remark:
          "One-time deposit of ₹5,000. Instant activation with Video KYC (nearest branch allocated)",
      },
      {
        bank: "Axis Bank",
        incentive: "₹500",
    ...
    "Mutual Funds": [
      {
        ...
        details:
          "The referral structure varies depending on the specific mutual fund.",
      },
    ],
    "Credit Card": [
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-emerald-50 to-cyan-100">
      ...
      <header className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white py-32 px-4 shadow-lg">
        ...
        <div className="max-w-7xl mx-auto flex flex-col items-center">
          ...
          {/* Larger, bolder text */}
          <p className="mt-3 text-lg text-center text-teal-100 max-w-3xl">
            {" "}
            {/* Subtitle */}
            Discover the lucrative referral incentives across our range of
            financial products.
          </p>
        </div>
      </header>
      ...
      <div className="bg-white/80 backdrop-blur-sm shadow-md sticky top-0 z-10 border-b border-emerald-100">
        {" "}
        {/* Updated tab bar background and shadow */}
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-1 py-3">
            {" "}
            {/* Centered tabs with wrapping and gap */}
            {tabs.map((tab) => {
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        {activeTab === "Stock Broking" && (
          <div className="space-y-6 mt-6">
            <h2 className="text-2xl font-bold text-emerald-800 border-b-2 border-emerald-200 pb-2">
              Stock Broking Referral Incentives
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {referralData["Stock Broking"].map((item, index) => (
                <div
                  key={index}
                  className="bg-white p-6 rounded-2xl shadow-xl border border-emerald-100 transition-transform duration-300 hover:shadow-2xl hover:-translate-y-1"
                >
          ...
          <div className="space-y-8 mt-6">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-2xl shadow-lg border border-blue-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {referralData["Loans"]["Personal Loan & Business Loan"].map(
                  (item, index) => (
                    <div
                      key={index}
                      className="bg-white p-4 rounded-xl shadow-sm border border-blue-100 flex flex-col items-center justify-center"
                    >
                      <span className="text-gray-600 text-center">
                        {item.amount}
                      </span>
                      <span className="text-blue-700 font-extrabold text-2xl mt-1">
                        {item.percentage}
                      </span>
                    </div>
                  )
            ...
            <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 p-6 rounded-2xl shadow-lg border border-purple-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {referralData["Loans"]["Home Loan"].map((item, index) => (
                  <div
                    key={index}
                    className="bg-white p-4 rounded-xl shadow-sm border border-purple-100 flex flex-col items-center justify-center"
                  >
                    <span className="text-gray-600 text-center">
                      {item.amount}
                    </span>
                    <span className="text-purple-700 font-extrabold text-2xl mt-1">
                      {item.percentage}
                    </span>
                  </div>
                ))}
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-6 rounded-2xl shadow-lg border border-amber-200 mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {referralData["Loans"]["Loan Against Property"].map(
                  (item, index) => (
                    <div
                      key={index}
                      className="bg-white p-4 rounded-xl shadow-sm border border-amber-100 flex flex-col items-center justify-center"
                    >
                      <span className="text-gray-600 text-center">
                        {item.amount}
                      </span>
                      <span className="text-amber-700 font-extrabold text-2xl mt-1">
                        {item.percentage}
                      </span>
                    </div>
                  )
        ...
        {activeTab === "Saving A/C" && (
          <div className="space-y-6 mt-6">
            <h2 className="text-2xl font-bold text-emerald-800 border-b-2 border-emerald-200 pb-2">
              Savings Account Referral Incentives
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {referralData["Saving A/C"].map((item, index) => (
                <div
                  key={index}
                  className="bg-white p-6 rounded-2xl shadow-xl border border-green-100 transition-transform duration-300 hover:shadow-2xl hover:-translate-y-1"
                >
                  <div className="flex justify-between items-start mb-4">
        ...
        {activeTab === "Credit Card" && (
          <div className="mt-6">
            <h2 className="text-2xl font-bold text-emerald-800 border-b-2 border-emerald-200 pb-2 mb-6">
              Credit Card Referral Incentives
            </h2>
            <div className="bg-gradient-to-br from-rose-50 to-pink-50 p-6 rounded-2xl shadow-lg border border-rose-200">
              <h3 className="font-bold text-xl text-rose-800 mb-4 text-center">
                Number of Cards vs. Incentive per Card
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
                {referralData["Credit Card"].map((item, index) => (
                  <div
                    key={index}
                    className="bg-white p-5 rounded-xl shadow-sm border border-rose-100 flex flex-col items-center justify-center"
                  >
                    <span className="text-gray-600 text-center font-medium">
                      {item.cards}
                    </span>
                    <span className="text-rose-700 font-extrabold text-2xl mt-2">
                      {item.incentive}
                    </span>
                  </div>
                ))}
              </div>
              <p className="text-sm text-rose-600 mt-6 text-center italic">
                T&C: Incentive will be given after issuing the card.
              </p>
            </div>
          </div>
        )}
