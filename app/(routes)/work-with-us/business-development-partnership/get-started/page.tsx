...
type TabId = 'Stock Broking' | 'Loans' | 'Saving A/C' | 'Mutual Funds' | 'Credit Card'| 'Insurance' | "Brochure";
...
const GetStartedPage = () => {
  ...
  const tabs: Array<{ id: TabId; label: string; icon: React.ComponentType<{ className?: string }> }> = [
    ...
    { id: 'Saving A/C', label: 'Savings Account', icon:  Banknote },
      ...
      <main className="w-full px-6 py-12">
        ...
        {activeTab === "Loans" && (
          <div className="space-y-8">
            <h2 className="text-3xl font-bold text-center text-gray-800">Loan Referral Commissions</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white rounded-2xl shadow-md border border-emerald-100 p-8">
                <h3 className="text-2xl font-bold text-emerald-700 mb-4">Personal & Business Loan</h3>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex justify-between items-center p-4 bg-emerald-50 rounded-lg">
                      <span className="font-medium">{item.amount}</span>
                      <span className="font-bold text-emerald-700 text-xl">{item.percentage}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-white rounded-2xl shadow-md border border-emerald-100 p-8">
                <h3 className="text-2xl font-bold text-teal-700 mb-4">Home Loan</h3>
                <div className="space-y-4">
                  {[
                    { amount: "Upto 2 Cr", percentage: "0.30%" },
                    { amount: "Above 2 Cr", percentage: "0.35%" },
                  ].map((item, i) => (
                    <div key={i} className="flex justify-between items-center p-4 bg-teal-50 rounded-lg">
                      <span className="font-medium">{item.amount}</span>
                      <span className="font-bold text-teal-700 text-xl">{item.percentage}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <div key={i} className="bg-white rounded-2xl shadow-md border border-emerald-100 p-6">
                  <h3 className="text-xl font-bold text-emerald-700 mb-3">{item.bank}</h3>
                  <div className="mb-4">
                    <span className="font-semibold text-gray-600">Incentive:</span>
                    <p className="text-2xl font-bold text-emerald-700">{item.incentive}</p>
                  </div>
        ...
        {activeTab === "Credit Card" && (
          <div className="space-y-8">
            <h2 className="text-3xl font-bold text-center text-gray-800">Credit Card Referral Incentives</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6">
              {[
                ...
                <div
                  key={i}
                  className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white p-6 rounded-2xl shadow-xl text-center group hover:scale-105 transition-transform"
                >
                  <div className="text-lg font-medium opacity-90">{item.cards}</div>
                  <div className="text-3xl font-extrabold mt-3">{item.incentive}</div>
                </div>
              ))}
            </div>
            <p className="text-lg text-gray-500 text-center italic">
              *Incentive per card, paid after issuance.
            </p>
          </div>
        )}
